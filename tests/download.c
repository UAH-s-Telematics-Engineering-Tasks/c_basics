...
#define TARGET_URL "http://calma.srg.uah.es/lolo"
...
int main(int argc, char* argv[]){
	...
	if(REMOTE_TARGET_SIZE_IN_BYTES % num_processes == 0){
		...
		printf("%d chunks of %ld bytes\n", 1, REMOTE_TARGET_SIZE_IN_BYTES -
		(chunk_size*num_processes) + chunk_size);
	}
	...
	for(i=1; i <= num_processes; i++){
		 ...
		 * debuggikng purpose (i.e. printf("\t chunk #%d: Range %d-%d \n", i,
		 * from, to))
		 * -Generate the name of the file for the current chunk, following the
		 * pattern download-1, download-2, download-i. You can do this with
		 * something like: sprintf(outfile, "%s-%d", CHUNK_FILENAME_PREFIX, i);
		...
		if(download_mode == 'S'){
			/**
			 * TODO: wait until the current chunk download has finished if the
			 * download mode is S (sequential)
			 */
			 wait(NULL);
             printf("The %d chunk has finished\n", i);
		}
	}
	if(download_mode == 'P'){
		/**
		 ...
		 */
         ...
         printf("All the chunks have finished, the download is complete\n");
	}
...
int are_arguments_correct(int argc, char* argv[]){
	 ...
	 */
	 if(argc != 3){
	 	printf("error: invalid nuber of arguments\n"
