...
{
  if (!p_bbdd || !nombre || !apellido || ingresos < 0)
    return -1;
  if (strlen(nombre) > MAX_NOMBRE -1 || strlen(apellido) > MAX_APELLIDO - 1 || p_bbdd->num_clientes >= MAX_ARRAY)
    return -1;
...
int ordenarPorNombreArrayPunterosInsercion(Cliente_t** pp_clientes, int n)
{
  ...
  for (int i = 1; i < n; i++) {
    ...
    //printf("1");
    //fflush(stdout);
    while ((aux_pos >= 0) && strcmp(tmp->nombre, pp_clientes[aux_pos]->nombre) > 0) {
      //printf("2");
      //fflush(stdout);
